...
public class DrawPdf417Activity extends Activity implements OnClickListener {
	...
	private void printPdf417() {
		...
		if(editText.getText().toString().equals("")){
			editText.setText("Label Printer");
		}
		...
		if (radioButtonId == R.id.radio7) {
			rotation = BixolonLabelPrinter.ROTATION_NONE;
		} else if (radioButtonId == R.id.radio8) {
			rotation = BixolonLabelPrinter.ROTATION_90_DEGREES;
		} else if (radioButtonId == R.id.radio9) {
			rotation = BixolonLabelPrinter.ROTATION_180_DEGREES;
		} else if (radioButtonId == R.id.radio10) {
			rotation = BixolonLabelPrinter.ROTATION_270_DEGREES;
		}

		BixolonPrinterPlugin.mBixolonLabelPrinter.beginTransactionPrint();
		BixolonPrinterPlugin.mBixolonLabelPrinter.drawPdf417(data, horizontalPosition, verticalPosition, maxRow, maxColumn, mErrorCorrectionLevel, compression,
